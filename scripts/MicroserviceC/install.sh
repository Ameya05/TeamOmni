...
sudo docker run -it --add-host=dockerhost:$(ip route | awk '/docker0/ { print $NF }') -e DOCKER_HOST=`curl http://169.254.169.254/latest/meta-data/public-hostname/` --name omniStormDetector -h storm.detector -p 8081:8080 -d omni/stormdetect:v1 >> /var/log/sga-docker.log 2>&1